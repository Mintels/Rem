...
DATA_FILE = os.path.join(BASE_DIR, "data", "dialogues.csv")
...
dialogues = pd.read_csv(DATA_FILE, dtype=str)

input_col = "input"  # CSV Column Name for Inputs.
output_col = "outputs"  # CSV Column Name for Target Responses.

def clean_column(column):  # Takes a Pandas Series, Removes Whitespace and Quotes
    return (
        column.astype(str)
            .str.strip()
...
class Encoder(nn.Module):
    ...
    def forward(self, src, hidden=None): 
        ...
        return outputs, hidden   # hidden is (h, c) for LSTM
...
def train():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for src, tgt in dataloader:
            ...
            _, hidden = encoder(src) # encoder hidden state (h, c)
